...
class KeplerTargetPixelFile(TargetPixelFile):
    ...
    def get_prior(self):
        ...
        in the tpf field.
        """
        # Get sources around the tpf
        field_stars = self.get_sources()

        # Generate priors based only on the tpf
        if len(field_stars) == 1:
            log.warning('Only once source has been identified. Priors will be generated by the TPF')

            # filer nans from centrois
            col, row = np.nanmedian(self.centroids(), axis=1)
            # Maximum flux from tpf
            MAXflux = np.nansum(self.flux, axis=(1, 2)).max()
            # List of prior
            tpf_prior = [col, row, MAXflux, self.keplerid]

            return tpf_prior

        # Generate priors in crowded fields
        elif len(field_stars) > 1:
            log.warning('More sources have been identified in the TPF.')

            # Convert source sky coordinates to column and row values
            col, row = self.wcs.wcs_world2pix(field_stars['ra'], field_stars['dec'], 0)
            # Return estimated flux of each source
            star_flux = kpmag_to_flux(field_stars['mag'], photometry='SAP')
            # Return Id of each source
